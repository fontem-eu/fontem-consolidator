...
body = json.dumps({'entity_type': '$entity_type', 'ids': $ids_json, 'triggered_by': 'full_scan'}).encode()
...
drain_file() {
  # $1 = entity_type (Company|Authority)
  # $2 = .todo file
  local entity_type="$1" file="$2" total done_n=0
  total=$(wc -l < "$file" | tr -d ' ')
  log "draining $file ($total remaining, batch=$BATCH_SIZE)"
  while [[ -s "$file" ]]; do
    local ids
